...
async function readOriginHistory(origin: string): Promise<{
  ...
  const { data, error } = await supabaseAdmin.from('wallet_recipe_queue')
...
export async function GET(request: NextRequest) {
  ...
  try { owner = getAddress(auth.evmAddress); } catch {
    return NextResponse.json({ error: '登录钱包地址无效' }, { status: 400 });
  }
  try {
    const [owned, origin] = await Promise.all([
      getOwnedEchoes(owner),
      readOriginHistory(owner),
    ]);
    ...
    return NextResponse.json({ error: '池中回声链上档案暂不可用' }, { status: 503 });
  }
}
