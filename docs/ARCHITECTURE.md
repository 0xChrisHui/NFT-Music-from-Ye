...
首页 = 全屏乐器 + 群岛
  ...
  → 曲子播完 / 点停止 → 提示"你的创作已记录，24h 可收藏"
  → 草稿存 localStorage（附 createdAt 时间戳，不需要登录）
  → 点爱心 → 登录 → 静默完成：铸造素材 NFT + 上传草稿 → "收藏成功"
  → 个人页：我的收藏（素材 NFT）/ 我的创作（草稿 + 24h 倒计时）
```

**用户全程不需要 ETH / AR / 签名。甚至不需要登录就能体验合奏。只有收藏时才需要登录。**
**设计原则：尽可能少打扰用户。用户只做一个动作（点爱心），背后的事自动完成。**
...
### 决策 16：收藏 = 铸造（爱心按钮）+ 状态机

用户看到的是"收藏"（岛屿上方的爱心），背后触发的是 NFT 铸造。

- "铸造"对普通用户陌生，"收藏"所有人都懂
- 不点爱心 = 不铸造 = 不需要登录，完全 OK
- **设计原则：少打扰。** 用户只做一个动作（点爱心），背后自动完成所有事

**爱心状态机（点击后的完整流程）：**

```
idle → [点击爱心]
  → auth_required（未登录）→ Privy 登录
     → 登录失败 → 回到 idle
     → 登录成功 ↓
  → processing（已登录）
     → 并行：铸造素材 NFT + 上传 localStorage 草稿（如有）
     → 全部成功 → favorited（爱心红色，显示"收藏成功"）
     → 铸造失败 → idle + 提示"收藏失败，请重试"
     → 草稿上传失败 → favorited（素材已铸造）+ 草稿留在 localStorage 等下次
```

**无草稿时点爱心**：允许。只铸造素材 NFT（Phase 1 已有能力），不上传草稿。
**有草稿时点爱心**：铸造素材 NFT + 上传草稿到后端，/me 同时显示两者。

### 决策 17：草稿存 localStorage，登录后上传

录制结束后草稿立即存浏览器 localStorage（附 `createdAt` 时间戳），不需要登录。

- 刷新页面 / 关浏览器 / 重启电脑：**不丢**
- 清浏览器缓存 / 换设备：**会丢**（即兴创作场景可接受）
- 用户点爱心 → 登录成功 → 自动从 localStorage 取草稿上传后端
- **24h TTL 从创作时间（createdAt）开始算**，不是从上传时间算
- `SaveScoreRequest` 必须包含 `createdAt`，服务端校验：超过 24h 的草稿拒绝（400）
- 前端也根据 `createdAt` 显示倒计时，过期草稿不展示上传按钮
- 登录后进入 `/me` 时，自动把 localStorage 中未过期草稿补上传到后端

**为什么不存服务端**：不需要匿名用户机制，不改后端，零延迟。

### ⚠️ AI 编码约定（不可绕过的硬约束）

上面 17 条决策是项目的**形状骨架**，AI 在写代码时不允许默默偏离：
...
### 收藏（乐观 UI，少打扰）
点爱心 → 登录 → 静默完成铸造+上传 → 爱心变红 + "收藏成功"
**只有彻底失败才弹通知**（极低概率）。用户只做一个动作。

### 个人页 /me — 两个区域
- **我的收藏**：已铸造的素材 NFT（爱心红色的那些曲目）
- **我的创作**：已上传的合奏草稿 + 24h 倒计时（Phase 3 可铸造为乐谱 NFT）
- 登录后进入 /me 时，自动把 localStorage 未过期草稿补上传

### 24h TTL 可视化
倒计时从 **createdAt（创作时间）** 算起，不是上传时间。
过期 = 沉入海底动画（不是静默删除）。

### 移动端策略（Phase 2）
**Phase 2 移动端 = 仅浏览，不可演奏。** 这是有意的产品取舍：
