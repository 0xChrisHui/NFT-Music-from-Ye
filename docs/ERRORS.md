...
### E040 — P11 浏览器缓存目录无法整体跨工作树移动

- 📅 2026-09-01 / 工作树堆积收口
- 😱 现象：精确校验源/目标路径后，`Move-Item` 仍对 `.tmp-p11-audit` 返回 Access denied，并留下空目标目录。
- 🧠 原因：目录包含 Edge profile、扩展与依赖缓存，部分文件仍受 Windows 进程或权限保护；不是 Git 数据损坏。
- 🔧 处理：不强删、不换 shell 绕过；只保留可读盘点文档，并把原始缓存目录加入 Git 忽略。其余未占用的 Phase 13 草稿、P9 过程证据和工具记忆按精确路径移动成功。
- 💡 临时目录的目标是退出版本控制，不一定要物理消失；被系统占用时“保留并忽略”比强制清空更安全。

### E041 — P9 生产冒烟复用浏览器且等待了错误的就绪信号

- 📅 2026-09-05 / P9 最终封存复验
- 😱 现象：第一次冒烟误连到已有 Edge 实例，随后新实例又因为等待音乐圆按钮而超时；生产页面与接口实际均为 HTTP 200。
- 🧠 原因：脚本只偏好 localhost 页面，并把音乐按钮是否出现当作 P9 演奏层就绪条件；这两项都不是生产首页的稳定契约。
- 🔧 修复：按生产域名优先选择 CDP 页面，并以页面出现“33 个声音动画”作为演奏层就绪信号；部署 SHA 改由环境变量注入，脚本不再写死历史版本。
- ✅ 结果：独立无扩展 Edge 中 K→FX11 成功、A 在无日食时按契约拒绝，首页浮窗与调参面板均不可见，控制台错误为 0。
- 💡 端到端夹具应等待产品契约而非偶然 DOM，并显式绑定目标域名和被测版本。
...
### E047 — rebase 自动合并留下重复的 Score 状态字段

- 📅 2026-09-06 / P11 发布分支同步
- 😱 现象：同步远端 13 个上线提交后，TypeScript 报 `OwnedScoreNFT` 的 `status` 为重复标识符。
- 🧠 原因：远端收藏页热修与 P11 档案重构都为同一接口补充了真实队列状态；Git 自动合并类型文件时保留了两份相同字段。
- 🔧 修复：保留带说明的唯一 `status: ScoreMintStatus`，删除后方重复声明；API 与档案组件继续共享同一字段。
- 💡 rebase 没有文本冲突不等于语义一定无冲突；合并后必须重新跑类型检查和完整验证。
