...
> 状态：G1–G6、G8 已冻结；G7 音频永久公开权仍待权利人确认，永久上传与网络发布保持关闭
...
| 项目 | 最终值 |
|---|---|
| 片段边界 | 60ms 等功率交叉淡化；E0 用真实片段复核，若出现可测削波或吞瞬态才回到本表改值 |
...
| 素材 | 当前证据 | 状态 |
|---|---|---|
| 36 个 MP3 及其中第三方采样 | 工作区有源工程与导出文件，但没有权利人的永久公开/NFT 使用确认 | **待确认** |
| 共用封面 | 将由本项目程序生成，不引入第三方图片；完成后记录生成规则与 hash | 待 E0/E5 产出 |
| Decoder | 项目自有代码、系统字体、程序纹理、现有许可依赖；上传前生成依赖清单 | 待 E2 审计 |

在权利人明确确认“拥有或已获授权，可将 36 段音频及其中采样永久公开并用于 NFT/播放器”之前，A1、E2-upload、F1 及其后的所有永久写入和链上发布不得执行。仅有本地文件不等于权利证明。
...
不依赖 G7 的 A0、B、C、E 本地 Foundation、D 本地代码与测试可以继续。G7 未关闭时，所有永久上传、真实 migration、网络部署、环境写入、cron 创建和 live 动作保持关闭，并在最终报告中给出精确恢复点。
