...
- **（历史·2026-07-05 冻结）Phase 8-L**：Wave 0+1 已合并入 main（35 个新 flag 默认 false）。两条待办线由用户定先后：
  ...
  - ② **P8-G /test1 GL 渲染层 spike**（2026-06-12 立项，**进行中**）：**G1-G5 ✅**（克隆+关透视+GL 基调骨架 / G4 GL 球 instanced+DOM 命中层+TunePanel `744b64d` / G5 程序化水面+视觉控制台 `eb5e11d`）；**G6-1 已实现**（水位 store+滚轮缓动+wheelMode 互斥+球淡出覆盖+水位指示，工作树）。**2026-06-14 方向再拍板 + 重组 G/H/I 三线**（水面改"动态扭曲"对标 jquery.ripples + 水位全局变量 + 鼠标交互；G 地基✅ / H 水面子系统 / I 去SVG+全新组件，G7 拍板取消、收口并入 I）。**H1–H6 ✅ = H 线全完**（RTT spike `a8b8adb` / 扭曲整合真场景 `7bd678c` / 水位深度遮罩 `1210c4c` / 涟漪交互全集 `191d34a` / 对象运动模型 `2065e25`+`9892be1` / H6 总验收[参数板布局+运动参数+reduced-motion+色斑修复 Nearest→Linear] `baddee8`+`a170029`）。规格 v1 主体全达；deferred 4 项（pinch/性能降级参数→移动端、默认水位滑块→按需、每实例 min/max+种子→I 线）。**I 线**：I1 ✅（去 SVG/卸 Archipelago+停 sim+GlNav `7469ae4`）+ I2 ✅（GL 日蚀 `add5a2f`）+ I3 ⏸ 暂缓（新组件待用户定）。**方向已定：GL 替换 SVG 首页**→ 新立 **J 线（生产化迁移）`playbook/phase-8/85-j-migration.md`**：**J1 ✅**（WebGL 兜底 `4abc882`+`967b66a`：不白屏 + Canvas 只挂一次防泄漏）/ **J2 ✅ 代码完成**（resize 对齐 `723419a` 桌面已验 / 触控 pinch+拖点 `911b702` 待真机验）/ **J3 ✅ 机制就位**（自动降配 `eb59ab3`+`58dce2e`：低 FPS 降 DPR/滞回，桌面确认 fire；真效果验真机）/ **J4 ✅**（加载提示+重试+预热 `efe1dd2`）。**J 线本期 J1–J4 全完 ✅ = GL 沙盒生产级靠谱**。deferred 已铸造记号/生产化/定稿/正式替换+删 SVG。**J 线本期已 push**。**K 线（水塘视觉/手感深化）— K1–K12 全部实现 ✅**（2026-06-18，`playbook/phase-8/90-k-visual-deepening.md`；均在 /test1 沙盒、ScenePanel 开关门控、默认关=现状、红线全守、verify=tsc+eslint）：K1 涟漪正圆（按屏比建格+9点拉普拉斯）/K2 划水路径插值/K3 深度三层模型（修 R4 水下球可见）/K4 浮出球投影（远光源软影，4 模式）/K5 月光焦散/K6 水面缩放（镜像平铺无真空）/K7 /test2 参考页/K8 漂浮微光/K9 睡莲/K10 可见塘底「亮底」（5 套花纹 暗矿/亮沙/虹彩/莲花/星河）/K11 月光倒影/K12 水位标尺柱（礁石/水晶，一点透视）。修复：播放暗斑（逐球遮罩原地淡出）+ 月光均匀眷顾水上水下。新组件 `pond-gl/decor/` + 花纹库 `pond-gl/shaders/pond-floor-shaders.ts`，参数进 RippleSpikePanel、开关进 ScenePanel。~~下一步 = 用户逐开关浏览器验收/调美拍板 → 定稿挑选入生产（J 线迁移上首页）~~ **2026-07-05 用户拍板：K 线验收 / J 线迁移 deferred / Wave 2 等收尾项冻结，收尾方案之后重新设计**。待真机验（同冻结）：J2 触控 + J3 降配真效果。旧版见 `70-g-gl-water.md` / `74-h-water-surface.md` / `80-i-desvg-components.md` / `85-j-migration.md` + `90-k-visual-deepening.md`。**当前 Now = P8-L 线（水塘生命感深化，沙盒 /test3）**：需求打磨定稿 2026-07-05，10 模块 spec 见 `playbook/phase-8/95-l-life-sense.md`（L0 基建 → L2 运动线[滚轮去同步·两参数/视差去同步/流场游移/偶发颤动] → L4 wake 扰球 → L5 呈现线[透明度隐现/光晕呼吸] → L3 形态线[能量球边缘/扰动激励/果冻感]；10 flag 默认 false + 参数进**新建 `life/LifePanel`**[非 RippleSpikePanel，原稿口径已修正]「生命感」5 组滑块）。**2026-07-05 L 线全部实现 ✅**（L0a 脚手架 + L0b 管道重构[per-node 深度/投影通道 + 拆 `spheres/sphere-frame.ts`] + L2/L3/L4/L5 十模块；新增 `life/`[life-core/life-tuning/LifePanel/flow-field/wake-field] 5 文件；10 flag 默认 false=像素级现状；逐文件 tsc+eslint 全绿、各文件≤220、`life/`5+`spheres/`8 文件硬线内；因账号 session rate-limit subagent 跑不动 → L0b 起主会话直接实现）。**下一步 = 用户逐开关浏览器验收 ⏸**（拖球命中对称专项 + 播放/hover 恒亮 + reduced-motion 静止 + 和谐压测）→ 拍板各幅度上限（L3-3 果冻感不过可弃）

---

## ⏭ Phase 7 启动序列（2026-05-13 重定，已完结归档）

Phase 7 playbook（已完结归档）：`playbook/phase-7/overview.md` + 3 个 track 子文档（覆盖重写，三方 review 整合）
...
- [x] A3+A12 score queue 状态机修复包（2026-05-16 commit `a48f4de`：双 mint 防御 mint/uri_attempted_at 窗口 + lease 25min 根因 status CAS 删除 + A8 Resend manual_review 邮件接入）
- [x] A8 Resend manual_review 告警（顺手并入 A3+A12，commit `a48f4de`）
- [x] A4 MAINNET-RUNBOOK grantRole 验收命令（§3.3.2 五项 cast call hasRole 验收块）
- [ ] A5 换 Turbo wallet（恢复 generate-eth-wallet.ts + 新 EOA + 充值）
- [x] A6 **15 曲** arweave_url 上链（2026-05-16；艺术家给 10 首新曲 No.6-15 + 原 5 首 = 15）
  - A6.0 资产盘点（tracks 表只有 5 行，B+C 靠前端循环 → 改为 INSERT）
  - A6.1 tracks INSERT week 6-15（migration 033）+ sphere-config 5→15 + SphereNode badge 双位数
  - A6.2 10 首 mp3 上 Arweave + arweave_url 回写（week 6-15 全部 OK）
- [x] A9 vercel-env-sync 脚本（scripts/vercel-env-sync.ts + npm run env-sync）
- [x] A10 /score 链上灾备 UI 降级壳（FallbackShell + 顺手修返回链接 /→/me）
- [x] A13 useEventsPlayback 首播 decode 时序（decodeReady + triggerDecode 拆分）
- [x] A14 ScoreCard 提交时间 + auto-polling 2min（useScoreNftPolling hook）
- [x] A15 useMintScore 失败回滚（60s 轮询确认入队，超时 → error 态 + 重试按钮）
- [x] A16 operator-lock TTL 30s→120s + Lua heartbeat + 生产 fail-closed
- [x] A17 mint_events upsert 补 error 检查 + throw
- [audit] A11 ✅ 已修 commit `0d75a93`（仅 curl audit）
- [audit] A18 ✅ 已修 commit `0d75a93`（仅 grep audit）
- [挂 P10] A7 operator wallet 主网 ETH 充值
- [挂 P10] A5 换 Turbo wallet（测试网暂挂；主网前硬门槛）
- [挂 P10] A16 heartbeat 接入长 cron step（当前 TTL 120s + Lua 能力已实现）
...
- [x] B4a 端到端技术冒烟（用户 2026-05-15 线下走完 7 步确认）
- [x] B4b 投资人 demo 协调（用户 2026-05-15 确认搞定）
- [x] Track B review 整合（claude 自审 `reviews/2026-05-15-phase-7-track-b-review.md` + codex `reviews/2026-05-15-phase-7-track-b-codex-review.md`；codex 2 条 P2→P1 已落地：useAuth ready 双源 + SEMI-DEMO-SCRIPT 步骤 6 改 DevTools/Etherscan 校验）
...
### Track D — 登录 UI 抄社区钱包 + Semi 默认 + /me 跳社区钱包按钮（1.3 天 / 4 step）
- [x] D1 LoginModal 重设计：移除 tab，默认 Semi，底部邮箱直跳 Privy（2026-05-15）
- [x] D2 SemiLogin 两阶段布局 + PinInput 6 格验证码（2026-05-15）
- [x] D3 /me 顶栏条件按钮（2026-05-15）
- [x] D4 verify.sh 全绿 + 浏览器 7 步实测确认 ✅（commit `5a2211a` 后已补）
...
Phase 6 v2 playbook：`playbook/phase-6/overview.md`（已完结，仅保留追溯）
...
- [废弃] **#8** 移动端首帧 — HomeJam 已 dead-code，主页改用 Archipelago，原 step 失效；移动端体验挂 Phase 8 UI 重设计
...
- [挂 P10] **A5** /score/[id] 链上灾备 — B8 P3 删 score-fallback.ts noop 残留，灾备方案待 P10 重新设计
...
- [x] **E2** — Semi 登录 ✅ **已落地 P7 Track B+D**（主网 Semi 策略见 Phase 10）
...
- 9 项 strict review P1（详 `reviews/2026-05-13-phase-7-playbook-review.md` 末尾"不在 P7 范围"段）→ 统一挂 Phase 10
